...
(defun first-come-first-played(player grid deadline &optional (moves (next-move player grid)) (previous-moves '()))
 (let ((move (car moves)))
   (cond
     ...
     ((check-for-invalid-move grid move)
         (feedback "~A is not a valid move!~%~%" move)
       (first-come-first-played player grid deadline (cdr moves) previous-moves) )
     (T
       (let ((new-grid (move-bead grid (car move) (cdr move))))
         (print new-grid)
         (if
           (or (is-solved new-grid) (is-not-solved new-grid))
             (create-output new-grid NIL (reverse (cons move previous-moves)))
           (first-come-first-played player new-grid deadline (next-move player new-grid)
              (cons move previous-moves) ) ) ) ) ) ) )
...
(defun depth-first-search(player grid deadline &optional (moves (next-move player grid)) (result (create-output grid)) (previous-moves '()))
  (cond
    ((> (now) deadline)
      (with-time-out
        (first-come-first-played player grid deadline (next-move player grid) previous-moves) ) )
    ...
    (T
      ...
      (let ((move (car moves)))
        (if (check-for-invalid-move grid move)
          (depth-first-search player grid deadline (cdr moves) result previous-moves)
          (let*
            ((new-grid (move-bead (copy-grid grid) (car move) (cdr move)))
             (result
               (depth-first-search player new-grid deadline (next-move player new-grid) result (cons move previous-moves)) ) )
            (if (or (output-winner result) (output-time-out result))
              result
              (depth-first-search player grid deadline (reverse (cdr moves)) result previous-moves) ) ) ) ) ) ) )
 ...
 (let ((board (grid-board grid)))
   (reverse
     (loop
       ...
       do NIL
       append (combination-finder board row) ) ) ) )

(defun combination-finder(board &optional (row 1) (col 1) (max-col (list-length board)) (combo-color NIL) (bucket '()))
 (combination-logic board row col bucket max-col
...
(defun combination-logic(board row col bucket max-col color left right up down upper-left)
 (if (> col max-col)
   bucket
   (combination-finder board row (1+ col) max-col
     color
     (if
       (and
